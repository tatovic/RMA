...
- [ ] Briše se **kompletan lokalni sadržaj Room baze** (predmeti, lokacije, kategorije, kursevi) — **BR-005**. Odnosi se na odjavu; istekla sesija čuva neposlat rad korisnika, vidi BR-005 ispod
...
**Odjava i istekla sesija nisu ista stvar** (razdvojeno u tiketu 28). Odjava je korisnikova odluka: on je tražio da ode i pristao je da mu lokalni podaci nestanu, pa se briše **sve**. Istekla ili poništena sesija (`401`, FR-009) nije njegova odluka — server je samo prestao da priznaje token. Tu se **čuva njegov još neposlat rad**: predmeti u stanju `PENDING_CREATE`/`PENDING_UPDATE`/`PENDING_DELETE`, zajedno sa kategorijama i lokacijama bez kojih ti redovi ne mogu da postoje. Sve ostalo je keš koji server ionako vraća i briše se.

Razlog je što offline rad (FR-097) namerno drži izmene samo lokalno dok ne budu poslate — to je **jedini primerak** tog unosa. Brisanje cele baze na `401` ga je uništavalo bez pitanja i bez poruke. Posle ponovne prijave sačuvani redovi odlaze serveru pri prvoj sinhronizaciji.

Vlasništvo se i dalje poštuje (OWN-07): čuvaju se isključivo redovi **tog** korisnika, push šalje samo njegove redove, a prijava drugog naloga na istom uređaju prvo ukloni sve što mu ne pripada.
...
| 21 | Kursna lista i valuta prikaza | 6 | [x] | 2026-08-31 | cec82c9 | ručno provereno na emulatoru (Pixel6_API36) uz pravi backend i pravi open.er-api.com: promena valute u Profilu (RSD→EUR) odmah preračunala Dashboard ukupnu vrednost (5.436.012,83 RSD → 46.334,22 EUR, tačan kurs), detalji predmeta u RSD prikazuju tačan preračunati iznos (BR-011), Room `exchange_rates` sadrži tačno šest keširanih valuta (BR-012). TTL-preskakanje mreže i offline pad na keš provereni kroz implementaciju, ne uživo (mreža nije isključivana u ovoj sesiji) |
| 22 | Praćenje garancija | 6 | [x] | 2026-09-01 | e96f645 | ručno provereno na emulatoru (Pixel6_API36) uz pravi backend: Dashboard kartica upozorenja sortirana po hitnosti (3, 4, 8, 9, 12, 15, 23, 24, 28 dana), lista inventara pokazuje sve četiri boje statusa (sivo/zeleno/crveno/narandžasto potvrđeno na "Kuhinjske stolice"/"Samsung Galaxy S23"/"Radni sto od hrasta"/"Kožne cipele"), detalji predmeta prikazuju sva četiri statusa tačnim tekstom i bojom ("Aktivna (još 103 dana)", "Istekla", "Nema informacije"), promena praga u Profilu sa 30 na 7 dana odmah suzila karticu upozorenja na dva predmeta, prag preživeo force-stop i ponovno pokretanje aplikacije (DataStore) |
| 23 | Statistika sa grafikonima | 6 | [x] | 2026-09-01 | d77e3cf | ručno provereno na emulatoru (Pixel6_API36) sa postojećim lokalnim podacima (63 predmeta/11 kategorija, sesija već prijavljena): ukupan broj/vrednost/kategorije/prosek tačni (46.334,22 EUR, 735,46 EUR), pie chart raspodele vrednosti i bar chart broja predmeta po kategorijama, tabela sortirana po vrednosti opadajuće do „Dokumenta" 0,00 EUR, najskuplji predmet („Kuhinjske stolice (set od 4)"), raspodela po sve četiri statusa garancije (15/9/12/24) tačnim bojama. Potvrđeno i u tamnoj temi (`cmd uimode night yes`) — sav tekst/osa/legenda čitljiv. Usput otkrivena i ispravljena dva bug-a van prvobitne implementacije: (1) legenda pie chart-a za 11 kategorija izlazila van ekrana i sekla se na pola — dodat `isWordWrapEnabled`; (2) kategorija bez vrednosti (0 EUR) crtala je nultu isečku čija se labela preklapala sa susednom — takve kategorije se sada izostavljaju iz pie chart-a (ostaju u tabeli/bar chart-u). Naknadno pokrenut pravi backend (Node + MySQL, već zasejana baza) i uživo proveren prazno stanje: registrovan nov nalog („Test Prazan"), Statistika odmah po prijavi prikazuje „Nema podataka za statistiku" sa objašnjenjem umesto praznih grafikona, potvrđeno i u svetloj i u tamnoj temi. Posle provere odjava i ponovna prijava na demo nalog vratila svih 63 predmeta (podaci žive na serveru, odjava briše samo lokalnu Room bazu — BR-005) |
...
| 27 | Poliranje i provera projektnih zahteva | 7 | [x] | 2026-09-02 | c4f422e | ručno provereno na emulatoru (Pixel6_API36) uz pravi backend/MySQL: dodate ikonice na svim prazno/greška stanjima, dodata Error stanja na Lokacije/Statistika, popravljen stvaran bug gubitka unosa pri rotaciji na formi za dodavanje/izmenu predmeta (stanje premešteno u ViewModel, uživo potvrđeno rotacijom), dodata i testirana prava Room migracija (`MIGRATION_1_2`, indeks za NFR-01) sa `MigrationTest` na uređaju, tamna tema proverena na svih 13 fragmenata uključujući grafikone, lista testirana na 513 predmeta (`dumpsys gfxinfo`, 1.01% janky frames), CATEGORY_IN_USE/LOCATION_IN_USE sada nose stvaran broj predmeta a VALIDATION_ERROR stvarne poruke po polju (oboje pokriveno `SafeApiCallTest`), potvrđeno da nema hardkodovanog teksta, da mrežno logovanje ne radi u release build-u i da nijedan upit na serveru ne radi bez provere vlasnika. Test podaci i test admin nalog uklonjeni posle provere, demo nalog vraćen na zvaničnu seed vrednost |

| 28 | Popravke posle revizije koda | 8 | [ ] | 2026-09-03 | 01accd6 | **delimično** — sav kod, testovi i dokumentacija su gotovi i provereni: backend suite 35/35 nad pravim MySQL-om (regresije za sva četiri blokirajuća nalaza, OWN-01…07, granica autentifikacije), Android 78/78 unit i 20/20 instrumentisanih (`MigrationTest` prolazi nepromenjen — Room je ostao na verziji 2), obe MySQL migracije primenjene na razvojnu bazu i idempotentne. Uživo provereno na emulatoru (Pixel6_API36) uz pravi backend: nalaz 02 (predmet u novoj lokaciji, brisanje predmeta, sync, pa brisanje lokacije — 204 umesto trajnog 500, bez `SQLiteConstraintException`, kaskada uklonila i tombstone red iz MySQL-a), nalaz 03 (sat emulatora vraćen 10 minuta unazad, izmena predmeta prošla kao `PUT 200` a ne 409 i stigla u MySQL), nalaz 05 (avionski režim — greška za ~3s uz nula mrežnih pokušaja), nalaz 09 (dvostruki dodir na „Sačuvaj" — tačno jedan POST i jedan red), nalaz 14 (nijedan sadržaj ispod sistemskih traka, pet ekrana). Release build sa uključenim R8 instaliran i provozan kroz sve ekrane; pritom otkrivena i ispravljena stvarna R8/Gson regresija (`JwtUtils.JwtPayloadDto` je van `dto` paketa, R8 preimenovao polje `exp`, pa je prijava upadala u petlju bez ijedne poruke) i zatečen `assembleRelease` koji je i pre ovog tiketa padao na `lintVitalRelease` (10x `MissingDefaultResource` za `md_theme_dark_*`). **Nedostaje (TRK-03):** uživo scenario za nalaz 01 (>501 predmet, dva uređaja — pokriveno automatskim testom nad pravim MySQL-om, ali nije odigrano na uređajima), uživo scenario za nalaz 04 (dva uređaja istovremeno — traži drugi emulator) i ponovna provera tamne teme posle izmene tema |
